...
### Task 4 — Bank Transaction Processor (Thread Pool)

Build a complete transaction processing system using a thread pool.

**`TransactionProcessor` class:**

| Method                                  | Description                                                 |
| --------------------------------------- | ----------------------------------------------------------- |
| `TransactionProcessor(int num_threads)` | Creates a thread pool with `num_threads` workers            |
| `submit(Transaction)`                   | Adds a transaction to the pool's work queue                 |
| `wait_all()`                            | Blocks until all submitted transactions have been processed |
| `get_statistics() const`                | Returns a `Stats` struct (see below)                        |
...
struct Stats {
    int    total_submitted;
    int    total_processed;
    int    total_failed;           // e.g. insufficient funds
    double total_amount_processed;
...
- Use the `ThreadPool` from section 6 as the underlying executor (or build your own)
- Each transaction task should record its start and end time using `std::chrono::steady_clock` to contribute to `average_processing_time_ms`
- Use `std::atomic` for all statistics counters (no mutex needed for individual increments)
- `wait_all()` should use a `std::condition_variable` that fires when `total_processed + total_failed == total_submitted`

```cpp
Bank bank;
std::string a1 = bank.create_account("Alice", 5000.0);
std::string a2 = bank.create_account("Bob",   3000.0);

TransactionProcessor processor(4);

for (int i = 0; i < 50; ++i)
    processor.submit({a1, a2, "transfer", 100.0});

processor.wait_all();

Stats s = processor.get_statistics();
std::cout << "Processed: "       << s.total_processed        << "/50\n";
std::cout << "Failed: "          << s.total_failed            << "\n";
std::cout << "Avg time (ms): "   << s.average_processing_time_ms << "\n";
```

> **Hint:** use `std::atomic<int>` for `total_processed` and `total_failed`. After incrementing either, check if `total_processed + total_failed == total_submitted` and call `cv.notify_all()` if so.
