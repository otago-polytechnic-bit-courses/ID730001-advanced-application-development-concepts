...
| Method                  | Description                                                 |
| ----------------------- | ----------------------------------------------------------- |
| `__init__(num_threads)` | Creates a `ThreadPoolExecutor` with `num_threads` workers   |
...
| `get_statistics()`      | Returns a `Stats` dataclass (see below)                     |
...
class Stats:
    total_submitted:            int
    total_processed:            int
    total_failed:               int      # e.g. insufficient funds
...
- Use `concurrent.futures.ThreadPoolExecutor` as the underlying executor
- Each transaction task should record its start and end time using `time.perf_counter()` to contribute to `average_processing_time_ms`
- Use `threading.Lock` to protect the statistics (Python has no built-in atomic int, but `queue.Queue` size operations and integer `+=` under a lock are safe)
- `wait_all()` should use `concurrent.futures.wait()` on the collected futures

```python
bank = Bank()
a1 = bank.create_account("Alice", 5000.0)
a2 = bank.create_account("Bob",   3000.0)

processor = TransactionProcessor(4)

for i in range(50):
    processor.submit(Transaction(a1, "transfer", 100.0))

processor.wait_all()

s = processor.get_statistics()
print(f"Processed: {s.total_processed}/50")
print(f"Failed: {s.total_failed}")
print(f"Avg time (ms): {s.average_processing_time_ms:.2f}")
```

> **Hint:** store each `Future` returned by `executor.submit()` in a list. Pass the list to `concurrent.futures.wait()` in `wait_all()` to block until all futures are done.
