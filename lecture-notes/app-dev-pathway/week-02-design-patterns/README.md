...
The Decorator pattern is a structural design pattern that lets you dynamically add behaviour to an object by wrapping it, without modifying the original class or affecting other objects of the same type. Decorators implement the same interface as the object they wrap, so they are interchangeable.

In War, we can use the Decorator pattern to add optional features to a player — such as score tracking, or card counting — by wrapping an existing player object.
...
class PlayerDecorator(IPlayer):
    ...
    def has_cards(self):
        return self.wrapped.has_cards()

    def deck_size(self):
        return self.wrapped.deck_size()

    def play_card(self):
        return self.wrapped.play_card()

    def receive_card(self, card):
        self.wrapped.receive_card(card)


class ScoreTrackingDecorator(PlayerDecorator):
    def __init__(self, player):
    ...
    def print_stats(self):
        print(f"{self.get_name()} has won {self.cards_won} cards total.")


class CardCountingDecorator(PlayerDecorator):
    def __init__(self, player):
        super().__init__(player)
        self.count = 0

    def play_card(self):
        card = self.wrapped.play_card()
    ...
    def print_count(self):
        print(f"{self.get_name()} running count: {self.count:+d}")


alice = ScoreTrackingDecorator(BasePlayer("Alice"))
bob = CardCountingDecorator(BasePlayer("Bob"))
...
for card in deck[26:]:
    bob.receive_card(card)

for round_num in range(1, 11):
    if not alice.has_cards() or not bob.has_cards():
        break

    c1 = alice.play_card()
    c2 = bob.play_card()

    print(f"Round {round_num:2}: {alice.get_name()} plays {c1}  vs  {bob.get_name()} plays {c2}")
